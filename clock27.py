...
#    print('panel = {p:s} {w:d} x {h:d}  version={v:s} COG={g:d} FILM={f:d}'.format(p=epd.panel, w=epd.width, h=epd.height, v=epd.version, g=epd.cog, f=epd.film))
    ...
    if drm.framebuffer.width == 264:
        settings = Settings27()
    elif drm.framebuffer.width == 200:
        settings = Settings20()
    else:
...
def demo(drm, settings):
    ...
    while True:
        ...
        # border
        draw.rectangle([(1, 1), (width - 1, height - 1)], fill=None, outline='white')
        draw.rectangle([(2, 2), (width - 2, height - 2)], fill=None, outline='white')

        # date
        draw.text((settings.DATE_X, settings.DATE_Y), '{y:04d}-{m:02d}-{d:02d}'.format(y=now.year, m=now.month, d=now.day), fill='white', font=date_font)
        # day
        draw.text((settings.WEEKDAY_X, settings.WEEKDAY_Y), '{w:s}'.format(w=DAYS[now.weekday()]), fill='white', font=weekday_font)

        # hours
        draw.text((settings.X_OFFSET, settings.Y_OFFSET), '{h:02d}'.format(h=now.hour), fill='white', font=clock_font)
        ...
        draw.rectangle((colon_x1, colon_y1 - settings.COLON_GAP, colon_x2, colon_y2 - settings.COLON_GAP), fill='white', outline='white')
        draw.rectangle((colon_x1, colon_y1 + settings.COLON_GAP, colon_x2, colon_y2 + settings.COLON_GAP), fill='white', outline='white')

        # minutes
        draw.text((settings.X_OFFSET + width / 2, settings.Y_OFFSET), '{m:02d}'.format(m=now.minute), fill='white', font=clock_font)
